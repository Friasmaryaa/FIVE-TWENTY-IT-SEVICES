...
<?php
$id = isset($_GET['id']) ? intval($_GET['id']) : 0; // ✅ This defines $id
?>
<?php 
$ticket = $conn->query("SELECT t.*, concat(c.lastname, ', ', c.firstname, ' ', c.middlename) as cname, d.name as dname 
                        FROM tickets t 
                        INNER JOIN customers c ON c.id = t.customer_id 
                        INNER JOIN departments d ON d.id = t.department_id  
                        WHERE t.id = $id");

if ($ticket && $ticket->num_rows > 0) {
